...
data = pd.read_csv("users_dataset.csv")
dataset = data.replace(np.nan, 0, regex=True)
dataset['followers_count'] = dataset['followers_count'].astype(int)
dataset['gender_accuracy'] = dataset['gender_accuracy'].astype(int)
select_rows = dataset.loc[dataset['gender_accuracy'] >= 80] #selecte rows with accuracy>80
select_rows['gender'][select_rows['gender'] == 'female'] = '1'
select_rows['gender'][select_rows['gender'] == 'male'] = '2'

x = select_rows.drop(['_id', 'username', 'name', 'after_name', 'gender', 'gender_accuracy'], axis=1).values
y = select_rows['gender'].values
